...
static void reshow(int p, BOOL changed, int showoff, uschar *prompt)
{
int i;
...
void scrn_rdline(BOOL escape_flag, BOOL stack_flag, uschar *prompt)
{
  ...
  as "end of command". */
    ...
    switch (key)
      ...
      if (p > 0)
        {
        while (--p > 0 && 
          (ch_tab[cmd_buffer[p]] & ch_word) == 0);
        while (p > 0 && 
          (ch_tab[cmd_buffer[p]] & ch_word) != 0) p--;
        if ((ch_tab[cmd_buffer[p]] & ch_word) == 0) p++;
        }
      ...
      case ka_dtwr:             /* Delete to word right */
        ...
        if (pp >= pmax) pp = pmax;
        while (pp < pmax && 
          (ch_tab[cmd_buffer[pp]] & ch_word) != 0) pp++;
        while (pp < pmax && 
          (ch_tab[cmd_buffer[pp]] & ch_word) == 0) pp++;
        if (pp > p)
          {
      ...
      if ((1 <= key  && key <= max_keystring) ||
        ...
        {
        uschar *keydata = (key < 256 - max_fixedKstring)?
