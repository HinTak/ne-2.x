...
{
uschar *keychars = "abcdefghijklmnopqrstuvwxyz[\\]^_";
int i;
int offset = 0;
int end = 31;
int f = 0;
...
switch (cmd->misc)
  ...
  case show_buffers:
    ...
    while (b != NULL)
      ...
      uschar *changed = b->changed?  "(modified)" : "          ";
    ...
    if (cut_buffer != NULL)
      ...
      uschar *pastext = cut_pasted? "(pasted)  " : "          ";
      uschar *type = (cut_type == cuttype_text)? "<text>" : "<rectangle>";
      uschar *pad;
      int n = 0;
  ...
  case show_wordcount:
    ...
    long int lc = -1, wc = 0, cc = -crlfcount;   /* allow for eof "line" */
...
if (main_undelete != NULL)
  ...
  if ((main_undelete->flags & lf_udch) != 0)
    {
    uschar ch = main_undelete->text[--(main_undelete->len)];
