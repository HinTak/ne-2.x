...
/* Copyright (c) University of Cambridge 1990 - 2004 */
...
static int arg_setup_values(int argc, char **argv, int *a_argindex,
  int *a_argnum, LONGINT argflags, arg_result *results, uschar *arg,
    int present_value)
{
int argnum = *a_argnum;
      ...
      if (keystring[i+1] == '=')
        ...
        if (sscanf(keystring+i+2, "%d%n", &(results[argmax].data.number), &n) == 0)
          return arg_error(results, findkey(keynumber, keystring,
            (uschar *)(results+2)), "is followed by an unknown option"), -1;
        i += n + 1;
        }
      break;
      }
