...
while (p != NULL) 
  {
  freeblock HUGE *next = (freeblock HUGE *)(((uschar HUGE *)p) + p->free_block_length);
...
while(p != NULL)
  ...
  if (truebytesize <= p->free_block_length)
    ...
    if (leftover == 0)
      ...
      {  /* use bottom of block */
      freeblock HUGE *remains = (freeblock HUGE *)(((uschar HUGE *)p) + truebytesize);
...
if (s == NULL) return NULL; else
  {
  uschar *yield = store_Xget(strlen(s)+1);
  strcpy(yield, s);
  return yield;
  }
}
...
end = (freeblock HUGE *)((uschar HUGE *)start + length);
...
if (previous != store_freequeue)
  {
  freeblock HUGE *prevend = (freeblock HUGE *)(((uschar HUGE *)previous) + previous->free_block_length);
  if (prevend > start)
    { 
    #ifdef TraceStore
...
end = (block HUGE *)(((uschar HUGE *)start) + bytesize);
