...
BOOL init_init(FILE *fid, uschar *fromname, int filetype, uschar *toname, int stream)
{
FILE *to_fid = NULL;
...
/* If the flag main_stream is set, we are initializing for a non-windowing run
in stream mode, in which we have to hold the output open all the time. The fact
of stream mode is indicated by a non-NULL to_fid field in the buffer. */
...
init_buffer(main_bufferchain, 0, store_copystring(toname), 
  store_copystring(toname), from_fid, to_fid, main_rmargin);
main_nextbufferno = 1;
...
printf("-noinit     don\'t obey .neinitrc file\n");
...
strcpy(argstring,
  "from/"
  XSTR(MAX_FROM)
  ",to/k,id/s,help=h/s,line/s,with/k,ver/k,"
  "opt/k,noinit/s,tabs/s,tabin/s,tabout/s,notabs/s,binary=b/s,"
  "stream/n=1000,readonly=r/s");
#undef STR   
#undef XSTR
...
uschar *delims    = US",.:\'\"!+-*/";
uschar *printing  = US" ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
for (i = 0; i < (int)strlen((const uschar *)delims); i++)
...
for (i = 0; i < (int)strlen((const uschar *)printing); i++) 
...
{
FILE *f = sys_fopen(filename, "r", NULL);
if (f == NULL) error_moan(5, filename);
while (fgets(cmd_buffer, cmd_buffer_size, f) != NULL) cmd_obey(cmd_buffer);
fclose(f);
}
...
{
if (main_tabs == NULL || strcmp(main_tabs, "notabs") == 0) return;
if (strcmp(main_tabs, "tabs") == 0) main_tabin = main_tabflag = TRUE;
else if (strcmp(main_tabs, "tabin") == 0) main_tabin = TRUE;
else if (strcmp(main_tabs, "tabout") == 0) main_tabout = TRUE;
else if (strcmp(main_tabs, "tabinout") == 0) main_tabin = main_tabout = TRUE;
}
