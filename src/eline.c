...
void line_insertch(linestr *line, int col, uschar *ptr, int count, int padcount)
{
int i;
int oldlen = line->len;
...
if (count > 0)
  ...
  if (forwardsflag)
    {
    int cc;
    uschar *t = main_undelete->text + main_undelete->len;
    for (cc = 0; cc < count; cc++) { *t++ = TRUE; *t++ = *s++; }
    }
  else
    {
    int cc;
    uschar *t = main_undelete->text + main_undelete->len + 2*count - 1;
